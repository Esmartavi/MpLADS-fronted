  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <span className="px-2.5 py-0.5 text-[10px] font-mono font-semibold uppercase tracking-wider rounded-md bg-cyan-500/15 text-cyan-300 border border-cyan-500/30 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-3 self-start lg:self-auto">
            <button
              ...
              disabled={loading}
              className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white transition-all flex items-center gap-1.5 text-xs font-medium"
            ...
            <button
              ...
              disabled={isScanning}
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-500 hover:to-sky-500 text-white font-semibold text-xs border border-cyan-400/40 shadow-glow-cyan transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
            >
        ...
        {scanStatusMsg && (
          <div className="mt-4 p-3 rounded-xl bg-cyan-950/60 border border-cyan-800/80 text-xs text-cyan-200 flex items-center gap-2 animate-in fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400">PDFs Ingested</span>
            <FileText className="w-4 h-4 text-cyan-400" />
          </div>
          <div className="text-2xl font-bold font-display text-white">{totalScanned}</div>
          <div className="text-[11px] text-cyan-400 font-mono">100% Neural OCR Parsed</div>
        </div>

        <div className="glass-panel p-4 rounded-xl border border-rose-900/40 bg-rose-950/10 space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-rose-300">Amount Mismatches</span>
            <AlertOctagon className="w-4 h-4 text-rose-400" />
          </div>
          <div className="text-2xl font-bold font-display text-rose-400">{mismatchCount}</div>
          <div className="text-[11px] text-rose-400 font-mono">₹27.7 Lakh Discrepancy</div>
        </div>

        <div className="glass-panel p-4 rounded-xl border border-amber-900/40 bg-amber-950/10 space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-amber-300">Cross-Scheme Fraud</span>
            <AlertTriangle className="w-4 h-4 text-amber-400" />
          </div>
          <div className="text-2xl font-bold font-display text-amber-400">{crossSchemeCount}</div>
          <div className="text-[11px] text-amber-300 font-mono">MLA Double-Dipping</div>
        </div>

        <div className="glass-panel p-4 rounded-xl border border-indigo-900/40 bg-indigo-950/10 space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-indigo-300">Vendor Discrepancies</span>
            <Building2 className="w-4 h-4 text-indigo-400" />
          </div>
          <div className="text-2xl font-bold font-display text-indigo-400">{vendorDiscrepancyCount}</div>
          <div className="text-[11px] text-indigo-300 font-mono">Unregistered Entities</div>
        </div>
        ...
        <div className="glass-panel p-4 rounded-xl border border-emerald-900/40 bg-emerald-950/10 space-y-1 col-span-2 lg:col-span-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-emerald-300">Central MPLADS Clean</span>
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
          </div>
          <div className="text-2xl font-bold font-display text-emerald-400">
            {totalScanned - criticalCount}
          </div>
          <div className="text-[11px] text-emerald-300 font-mono">Verified Statutory Header</div>
        </div>
      </div>

      {/* Filter and Search Bar */}
      <div className="glass-panel p-4 rounded-xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 no-scrollbar">
          <span className="text-xs font-semibold text-slate-400 flex items-center gap-1 mr-2 flex-shrink-0">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full bg-slate-900/80 border border-slate-700/80 rounded-lg pl-9 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-7 space-y-3">
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-[11px] font-mono text-slate-400">Click any document to inspect forensics</span>
          </div>
              ...
              {filteredDocs.map((doc, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2.5">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700 truncate max-w-[220px]">
                            {doc.pdf_file}
                          </span>
                          {hasCrossScheme && (
                          ...
                          {isDuplicateRecycled && (
                            <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-rose-500/20 text-rose-300 border border-rose-500/30 flex items-center gap-1">
                        ...
                        <div className="text-xs text-slate-300 font-medium">
                          <strong>{doc.portal_record?.mp_name}</strong>
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80 font-mono text-[11px]">
                      <div>
                        <div className="text-[10px] text-slate-500 uppercase">Portal Disbursed</div>
                      ...
                      <div>
                        <div className="text-[10px] text-slate-500 uppercase">Paper Approved</div>
                        <div className={`font-bold ${doc.paper_extracted?.approved_amount ? 'text-cyan-300' : 'text-slate-500'}`}>
                      ...
                      <div>
                        <div className="text-[10px] text-slate-500 uppercase">Discrepancy Δ</div>
                        <div
                          className={`font-bold ${
                    ...
                    {isDuplicateRecycled && (
                      <div className="mt-2.5 px-3 py-1.5 rounded-lg bg-rose-950/30 border border-rose-800/50 text-[11px] text-rose-200 flex items-center justify-between">
                        <span>
                    ...
                    {hasCrossScheme && (
                      <div className="mt-2.5 px-3 py-1.5 rounded-lg bg-amber-950/30 border border-amber-800/50 text-[11px] text-amber-200">
                    ...
                    <div className="mt-2.5 flex items-center justify-between text-[11px] text-slate-400">
                      ...
                      <div className="flex items-center gap-1 text-cyan-400 hover:text-cyan-300 font-semibold">
        ...
        <div className="xl:col-span-5 space-y-4">
          ...
          {!selectedDoc ? (
            ...
            <div className="glass-panel p-5 rounded-2xl border border-cyan-500/30 space-y-5 sticky top-24 shadow-2xl bg-navy-950/80 backdrop-blur-xl">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-end justify-between p-3 opacity-90">
                      <span className="text-[10px] font-mono text-cyan-300 bg-slate-900/80 px-2 py-0.5 rounded border border-cyan-900">
                        {selectedDoc.image_file}
                      </span>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2">
                    <div className="text-[10px] font-mono font-semibold uppercase text-cyan-400 border-b border-slate-800 pb-1 flex items-center justify-between">
                      <span>Portal Record</span>
                      <span className="text-[9px] text-slate-500">MoSPI API</span>
                    </div>
                    ...
                    <div className="space-y-1.5 text-[11px]">
                      ...
                      <div>
                        <span className="text-slate-500 block text-[10px]">Scheme Registered:</span>
                        <span className="text-slate-300 font-medium">MPLADS (Sansad Nidhi)</span>
                      </div>

                      <div>
                        <span className="text-slate-500 block text-[10px]">Implementing Agency:</span>
                      ...
                      <div>
                        <span className="text-slate-500 block text-[10px]">Work Description:</span>
                        <span className="text-slate-400 line-clamp-2 italic text-[10px]">
                  ...
                  <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2">
                    <div className="text-[10px] font-mono font-semibold uppercase text-emerald-400 border-b border-slate-800 pb-1 flex items-center justify-between">
                      <span>Paper Certificate</span>
                      <span className="text-[9px] text-slate-500">RapidOCR Stamped</span>
                    </div>
                    ...
                    <div className="space-y-1.5 text-[11px]">
                      <div>
                        ...
                        <span
                          className={`font-bold font-mono ${
                            selectedDoc.paper_extracted?.approved_amount ? 'text-cyan-300' : 'text-slate-500'
                          }`}
                        >
                      ...
                      <div>
                        <span className="text-slate-500 block text-[10px]">Scheme Stated:</span>
                        <span
                      ...
                      <div>
                        <span className="text-slate-500 block text-[10px]">Extracted Vendor / Beneficiary:</span>
                        <span className="text-cyan-300 font-mono truncate block">
                          {selectedDoc.paper_extracted?.vendor_name || 'Not detected in stamp block'}
                        </span>
                      </div>

                      <div>
                        <span className="text-slate-500 block text-[10px]">UTR / Account Ref:</span>
              ...
              {(() => {
                ...
                if (diff > 5000) {
                  return (
                    <div className="p-3 rounded-xl bg-rose-950/40 border border-rose-800/80 text-rose-200 text-xs space-y-1">
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                  {(selectedDoc.findings || []).map((finding, fIdx) => (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-slate-300/90 leading-relaxed pl-1">
